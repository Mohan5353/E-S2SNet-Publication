...
def train(file_path, save_dir, dropout_rate=0.3, sigma=-1, is_realnoisy=True, alpha=0.3, beta=0.05, gamma=0.5, n_step=1000, mask_prob=0.5, reg_layer='dec_conv2b', n_prediction=100):
    ...
    model = network.Punet.build_denoising_unet(noisy, 1 - dropout_rate, is_realnoisy, alpha, beta, gamma, mask_prob, reg_layer)
...
if __name__ == '__main__':
    ...
    for f in files:
        if os.path.isfile(f):
            train(f, args.save_dir, dropout_rate=args.dropout_rate, sigma=-1, is_realnoisy=True, alpha=args.alpha, beta=args.beta, gamma=args.gamma, n_step=args.n_step, mask_prob=args.mask_prob, reg_layer=args.reg_layer, n_prediction=args.n_prediction)
