...
def msr_cnr(im, box1, box2, box3, box4):
	...
	# im_fore3.show()
	# print(im_fore2.size)
	back_image = np.asarray(im_back)
	fore1_image = np.asarray(im_fore1)
	fore2_image = np.asarray(im_fore2)
	fore3_image = np.asarray(im_fore3)

	mean_back, std_back = std_mean_back(back_image)
	print(mean_back, std_back)
	mean_fore, std_fore = std_mean_fore(fore1_image, fore1_image, fore1_image)
