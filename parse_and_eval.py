...
def evaluate_method(method_name, pred_dir, raw_dir, truth_dir):
    ...
    for i in range(1, 19):
        ...
        im = Image.open(im_path).convert('L').resize((900, 450))
        input_im = Image.open(raw_path).convert('L').resize((900, 450))
        ...
        if os.path.exists(truth_path):
            t_im = Image.open(truth_path).resize((900, 450)).convert('L')
